// blogs.js - renders the dedicated Blogs page
...
function profileButtons(d) {
  const el = qs('#profile-buttons');
  if (!el) return;
  const socials = d.socials || [];
  const dev = socials.find(s => /dev\.to/i.test(s.href) || /dev/i.test(s.label || ''));
  const medium = socials.find(s => /medium\.com/i.test(s.href) || /medium/i.test(s.label || ''));
  const btn = (label, href) => `<a class="px-4 py-2 rounded-md border border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 inline-flex items-center gap-2" href="${href}" target="_blank" rel="noopener">${iconLink()}<span>${label}</span></a>`;
  const parts = [];
